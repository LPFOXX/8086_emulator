...
namespace lp::m8086 {
    ...
    struct Memory {
        ...
        Byte getByte(Register &CS, Register &IP) const {
            Byte byte = mMemory[GET_MEM_ADDRESS(CS, IP)];
            ++IP;
            return byte;
        }

        [[nodiscard]] Byte getByte(const uint32_t memAddress) const {
            return mMemory[memAddress];
        }

        Word getWord(Register &CS, Register &IP) const {
        ...
        ModRMByte getModRMByte(Register &CS, Register &IP) const {
    ...
    enum class FlagMasks : Word {
        CarryFlagMask = 1 << 0,
        ParityFlagMask [[maybe_unused]] = 1 << 2,
        AuxiliaryFlagMask [[maybe_unused]] = 1 << 4,
        ZeroFlagMask [[maybe_unused]] = 1 << 6,
        SignFlagMask [[maybe_unused]] = 1 << 7,
        TrapFlagMask [[maybe_unused]] = 1 << 8,
        InterruptFlagMask [[maybe_unused]] = 1 << 9,
        DirectionFlagMask [[maybe_unused]] = 1 << 10,
    ...
    struct Processor {
        ...
        [[nodiscard]] uint32_t getMemoryAddress(const RM addressingMode, const Word displacement = 0x0000) const {
    ...
    private:
        void ADDEbGb(bool isWordInstruction) {
            const ModRMByte modeByte = mMemory.getModRMByte(CS, IP);
            Register source = getRegister((Registers) modeByte.Reg);

            switch (modeByte.Mod) {
                case Mod::RegisterValue: {
                    Register destination = getRegister((Registers) modeByte.RM);
                    uint32_t result = destination + source;
                    setRegister(modeByte.RM, (Word) result, isWordInstruction);

                    ADDChangeFlags(result, destination, source, isWordInstruction);
                    break;
                }
                case Mod::NoDisplacement: {
                    if (modeByte.RM == RM::RM_BP) {
                        // 16bits displacement
                        ...
                        // no displacement
                        uint32_t memoryAddress = getMemoryAddress((RM) modeByte.RM, 0);
                        Byte memoryContent = mMemory.getByte(memoryAddress);
                        uint32_t result = memoryContent + source;
                ...
                case Mod::ByteDisplacement: {
                    ...
                    uint32_t memoryAddress = getMemoryAddress((RM) modeByte.RM, displacement);
                    Byte memoryContent = mMemory.getByte(memoryAddress);
                    uint32_t result = memoryContent + source;
